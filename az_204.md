...
- Exam prep: https://github.com/Azure-Samples/cosmos-dotnet-core-todo-app (relevant getting started project):
    ...
    - Use `DOTNET_ROOT=/usr/local/homebrew/opt/dotnet/libexec dotnet run` in the app directory to run it
    - NOTE: you need to install X86 dotnet over `axbrew` first and run ITerm2 in rosetta 2 mode (go to the programms
      folder, right click/Information and turn rosetta mode on)